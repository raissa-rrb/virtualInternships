...
/*
So i addeda vector because its API will enbale us to easily add elements 
but most importantly remove elements using iterators

Alos, we can access the size of a vector so no need to keep item_count
*/
...
public:
    ...
    void add_item() {
        ...
        items.push_back(new Item(name, quantity, price)); //new
    }
    ...
    void sell_item() {
        ...
        std::vector<Item*>::iterator cur = items.begin();

        for (; cur != items.end() ; cur++) {
            if ((*cur)->is_match(item_to_check)) {
                remove_item(cur);
                return;
            }
        }
    ...
    void remove_item(std::vector<Item*>::iterator item) { //new
        int input_quantity;
